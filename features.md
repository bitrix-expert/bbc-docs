...
Методы обработки исключений находятся в типаже Common (подключается автоматически во всех компонентах ББК). Перечислим их.
...
## Проверка и приведение параметров
...
2. Создадим шаблон для обычного режима.
...
<aside class="notice">
В компоненте `elements.list` параметры аякса (идентификатор, страница шаблона, перезагрузка заголовков и режим) вынесены в настройки компонента (`.parameters.php`).
</aside>
...
<aside class="notice">
Тег кеша устанавливается и сбрасывается автоматически, если в кешируемой области (в данном случае таковой является метод `executeMain()`) происходит обращение к инфоблоку через `CIBlockElement::GetList()`. `GetList()` обязательно должен возвращать идентификатор инфоблока (поле `IBLOCK_ID`). В компонентах `elements.list` и `element.detail` кеш помечается и сбрасывается автоматически.
</aside>
...
Генерация постраничной навигации реализуется типажом `Elements`. Параметры компонента, влияющие на генерацию:
...
## «Эрмитаж»
Функционал «Эрмитажа» реализуется типажом `Elements`. Просто подключите его.
